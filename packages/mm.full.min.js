...
this.element.readyState>2&&!this.element.error&&!this.element.ended},getDuration:function(){return this.element.duration},getCurrentSrc:function(){return this.element.currentSrc}};v.add("nativ","video",a.extend({_videoFullscreen:a.support.videoFullscreen,enterFullScreen:function(){if(!this._videoFullscreen)return false;try{this.element[h.enter]()}catch(d){return false}return true},exitFullScreen:function(){if(!this._videoFullscreen)return false;try{this.element[h.exit]()}catch(d){return false}return true}},
...
i.replace(y.A,"%26").replace(y.a,"%26").replace(y.e,"%3D").replace(y.q,"%3F"):i};(function(){a.support.flash9=false;a.support.flashVersion=0;var i=x.getPluginVersion("Shockwave Flash"),k=function(j){a.support.flash9=false;try{if(j&&typeof j.GetVariable!=="undefined"){var m=j.GetVariable("$version");j=x.getPluginVersion("",{description:m});a.support.flashVersion=parseInt(i[0]+"."+i[1],10);a.support.flash9=!!(j[0]>9||j[0]===9&&j[1]>=115)}}catch(q){}};if(i&&i[0])a.support.flashVersion=parseInt(i[0]+
"."+i[1],10);if(i[0]>9||i[0]===9&&i[1]>=115){a.support.flash9=true;a(function(){i=a("<object />",u).appendTo("body");k(i[0]);i.remove()})}else if(window.ActiveXObject)try{i=new ActiveXObject("ShockwaveFlash.ShockwaveFlash");k(i);i=null}catch(d){}})();var c={isTechAvailable:function(){return a.support.flash9},_extendJWLoad:function(i,k,d){if(i){d=d||this.element;x.extendWithData(d,k,["type","provider","stretching","bufferlength"]);if(!this.canPlaySrc(i)){if(!k.provider)k.provider=h[this.nodeName];
if(!k.type)k.type=h[this.nodeName]}return k}},_embed:function(i,k,d,j){var m=this.embedOpts.jwPlayer,q=a.extend({},m.vars,{file:i,id:k}),o=a.extend({name:k},m.attrs,u,!window.ActiveXObject?{data:m.path}:{}),r=a.extend({movie:m.path},m.params),w=[],B=this;this._extendJWLoad(i,q);if(d.poster)q.image=d.poster;q.autostart=""+d.autoplay;q.repeat=d.loop?"single":"false";q.controlbar=d.controls?"bottom":"none";if(!d.controls&&this.nodeName!=="audio"&&r.wmode===undefined)r.wmode="transparent";if(!d.controls&&
m.hideIcons&&r.wmode==="transparent"||m.hideIcons===true){q.icons="false";q.showicons="false"}if(r.wmode==="transparent"&&!q.screencolor&&!o.bgcolor){q.screencolor="ffffffff";o.bgcolor="#000000"}r.flashvars=[];a.each(q,function(A,z){r.flashvars.push(f(A)+"="+f(z))});a.each(m.plugins,function(A,z){w.push(z)});w.length&&r.flashvars.push("plugins="+w.join(","));r.flashvars=r.flashvars.join("&");j(x.embedObject(this.visualElem[0],k,o,r,v,"Shockwave Flash"));setTimeout(function(){var A=a("object",B.visualElem)[0];
if(!A||A.style.display==="none"&&a("> *",B.visualElem).length>1){a("div[bgactive]",B.visualElem).css({width:"100%",height:"100%"});B._trigger("flashblocker")}},9)},canPlaySrc:function(i){var k=x.fn.canPlaySrc.apply(this,arguments),d=-1;d=i.src||i;if(!k&&typeof d==="string"){d=d.indexOf("youtube.com/");if(d<15&&d>6)k="maybe"}return k},canPlayCodecs:["avc1.42E01E","mp4a.40.2","avc1.58A01E","avc1.4D401E","avc1.64001E","VP6","mp3","AAC"],canPlayContainer:["video/3gpp","video/x-msvideo","video/quicktime",
"video/x-m4v","video/mp4","video/m4p","video/x-flv","video/flv","audio/mpeg","audio/mp3","audio/x-fla","audio/fla","youtube/flv","jwplayer/jwplayer"]};x.add("jwPlayer","video",c);x.add("jwPlayer","audio",c)})(jQuery);
