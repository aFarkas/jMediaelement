...
true;a(function(){h=a("<object />",w).appendTo("body");k(h[0]);h.remove()})}else if(window.ActiveXObject)try{h=new ActiveXObject("ShockwaveFlash.ShockwaveFlash");k(h);h=null}catch(d){}})();var b={isTechAvailable:function(){return a.support.flash9},_extendJWLoad:function(h,k,d){if(h){d=d||this.element;y.extendWithData(d,k,["type","provider","stretching","bufferlength","streamer"]);k.file=(d.getAttribute("data-jwprefixsrc")||"")+k.file;d=y.getExt(h);var j=d=="m4r"?"video":this.nodeName;if(d=="m4r"||
!this.canPlaySrc(h)){if(!k.provider)k.provider=i[j];if(!k.type)k.type=i[j]}return k}},_embed:function(h,k,d,j){var l=this.embedOpts.jwPlayer,q=a.extend({},l.vars,{file:h,id:k}),o=a.extend({name:k},l.attrs,w,!window.ActiveXObject?{data:l.path}:{}),s=a.extend({movie:l.path},l.params),u=[],A=this;this._extendJWLoad(h,q);if(d.poster)q.image=d.poster;q.autostart=""+d.autoplay;q.repeat=d.loop?"single":"false";q.controlbar=d.controls?"bottom":"none";if(!d.controls&&this.nodeName!=="audio"&&s.wmode===undefined)s.wmode=
"transparent";if(!d.controls&&l.hideIcons&&s.wmode==="transparent"||l.hideIcons===true){q.icons="false";q.showicons="false"}if(s.wmode==="transparent"&&!q.screencolor&&!o.bgcolor){q.screencolor="ffffffff";o.bgcolor="#000000"}s.flashvars=[];a.each(q,function(z,B){s.flashvars.push(f(z)+"="+f(B))});a.each(l.plugins,function(z,B){u.push(B)});u.length&&s.flashvars.push("plugins="+u.join(","));s.flashvars=s.flashvars.join("&");j(y.embedObject(this.visualElem[0],k,o,s,x,"Shockwave Flash"));setTimeout(function(){var z=
a("object",A.visualElem)[0];if(!z||z.style.display==="none"&&a("> *",A.visualElem).length>1){a("div[bgactive]",A.visualElem).css({width:"100%",height:"100%"});A._trigger("flashblocker")}},9)},canPlaySrc:function(h){var k=y.fn.canPlaySrc.apply(this,arguments),d=-1;d=h.src||h;if(!k&&typeof d==="string"){d=d.indexOf("youtube.com/");if(d<15&&d>6)k="maybe"}return k},canPlayCodecs:["avc1.42E01E","mp4a.40.2","avc1.58A01E","avc1.4D401E","avc1.64001E","VP6","mp3","AAC"],canPlayContainer:["video/3gpp","video/x-msvideo",
"video/quicktime","video/x-m4v","video/mp4","video/m4p","video/x-flv","video/flv","audio/mpeg","audio/aac","audio/mp4","audio/x-m4a","audio/m4a","audio/mp3","audio/x-fla","audio/fla","youtube/flv","jwplayer/jwplayer"]};y.add("jwPlayer","video",b);y.add("jwPlayer","audio",b)})(jQuery);
