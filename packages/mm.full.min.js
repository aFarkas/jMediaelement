...
(function(a){var r=document.createElement("video"),t=a.multimediaSupport,u={apiActivated:1,apiDeActivated:1,mediareset:1,native_mediareset:1,totalerror:1,progresschange:1,jmeflashRefresh:1},x={native_mediareset:1,apiDeActivated:1,native_mediareset:1,apiActivated:1,timechange:1,progresschange:1,mmAPIReady:1,jmeflashRefresh:1},k={};if("enterFullScreen"in r&&r.supportsFullscreen){a.support.videoFullscreen=true;k.enter="enterFullScreen";k.exit="exitFullScreen"}else a.each(["webkit","moz","o","ms"],function(c,
e){if(e+"EnterFullScreen"in r&&r[e+"SupportsFullscreen"]){a.support.videoFullscreen=true;k.enter=e+"EnterFullScreen";k.exit=e+"ExitFullScreen";return false}});r=null;a.extend(t,{formatTime:function(c){return a.map([parseInt(c/60,10),parseInt(c%60,10)],function(e){return isNaN(e)?"--":e<10?"0"+e:e}).join(":")}});a.event.special.loadedmeta={add:function(c){var e=a(this).getMMAPI();if(e&&e.loadedmeta){e=a.extend({},e.loadedmeta);c.handler.call(this,e,e)}}};a.extend(t.fn,{_trigger:function(c){var e=c.type?
c:{type:c},j=e.type;switch(j){case "mmAPIReady":if(this.isAPIReady)return;this.isAPIReady=true;break;case "loadedmeta":this.loadedmeta=e;break;case "totalerror":this.totalerror=true;break;case "mediareset":this._bufferLoaded=this.totalerror=this.loadedmeta=false;break}if(!(!this.isAPIActive||this.totalerror&&!u[j])){!this.isAPIReady&&!u[j]&&this._trigger("mmAPIReady");if(c.type==="progresschange"){if(isFinite(this._bufferLoaded)&&isFinite(c.relLoaded)&&this._bufferLoaded>=c.relLoaded)return;this._bufferLoaded=
...
0)this._trigger({type:"loadedmeta",duration:this.element.duration});else if(this.element.networkState!==2||a.attr(this.element,"preload")==="none"&&!a.attr(this.element,"autoplay")||!a.attr(this.element,"srces").length)this._trigger("mmAPIReady")},play:function(){this.element.play()},pause:function(){this.element.pause()},muted:function(c){if(typeof c!=="boolean")return this.element.muted;this.element.muted=c},volume:function(c){if(!isFinite(c))return this.element.volume*100;this.element.volume=c/
100},currentTime:function(c){if(!isFinite(c))return this.element.currentTime;try{this.element.currentTime=c}catch(e){}},_mmload:function(){this.element.load?this.element.load():a(this.element).triggerHandler("error")},_isPlaying:function(){return!this.element.paused&&this.element.readyState>2&&!this.error&&!this.ended},getDuration:function(){return this.element.duration},getCurrentSrc:function(){return this.element.currentSrc}};t.add("nativ","video",a.extend({_videoFullscreen:a.support.videoFullscreen,
enterFullScreen:function(){if(!this._videoFullscreen)return false;try{this.element[k.enter]()}catch(c){}},exitFullScreen:function(){if(!this._videoFullscreen)return false;try{this.element[k.exit]()}catch(c){}}},d));t.add("nativ","audio",d);a.fn.getMMAPI=function(c){if(this[0]){var e=a.data(this[0],"mediaElemSupport");return c||!e||!e.name||!e.apis?e:e.apis[e.name]}};var h={onAPIReady:1,loadSrc:1};t.registerAPI=function(c){if(typeof c==="string")c=[c];a.each(c,function(e,j){if((e=t.apis.video.nativ[j])&&
!a.fn[j]&&a.isFunction(e)&&j.indexOf("_")!==0)a.fn[j]=function(){var q=arguments,m;this.each(function(){var l=a(this).getMMAPI();if(l)if(h[j]||l.isJMEReady()&&!l.totalerror&&(l.name!=="nativ"||a.support.mediaElements))m=l[j].apply(l,q);else{l.unAPIReady(j+"queue");l.onAPIReady.call(l,function(){l[j].apply(l,q)},j+"queue")}});return m===undefined?this:m}})};var f=[];a.each(t.apis.video.nativ,function(c){f.push(c)});t.registerAPI(f);t.fn._extend=function(c){var e=[];a.each(c,function(j,q){t.fn[j]=q;
e.push(j)});t.registerAPI(e)}})(jQuery);
(function(a){function r(f,c){f=a(f);var e={},j=f.attr("data-controls");e.mm=j?a("#"+j):a("video, audio",f).filter(":first");e.api=e.mm.getMMAPI(true)||e.mm.jmeEmbed(c.embed).getMMAPI(true);if(f.is(c.controlSel))e.controls=f;if(!e.controls||e.controls.hasClass(c.classPrefix+"media-controls")){e.controlsgroup=f;if(f[0]&&!e.api.controlWrapper&&a.contains(f[0],e.mm[0]))e.api.controlWrapper=f;e.controls=e.controls?a(c.controlSel,f).add(e.controls):a(c.controlSel,f);e.api.controlBar=e.controls.filter("."+
