...
e){if(e+"EnterFullScreen"in r&&r[e+"SupportsFullscreen"]){a.support.videoFullscreen=true;k.enter=e+"EnterFullScreen";k.exit=e+"ExitFullScreen";return false}});r=null;a.extend(t,{formatTime:function(c){return a.map([parseInt(c/60,10),parseInt(c%60,10)],function(e){return isNaN(e)?"--":e<10?"0"+e:e}).join(":")}});a.event.special.loadedmeta={add:function(c){var e=a(this).getJMEAPI();if(e&&e.loadedmeta){e=a.extend({},e.loadedmeta);c.handler.call(this,e,e)}}};a.extend(t.fn,{_trigger:function(c){var e=
c.type?c:{type:c},j=e.type;switch(j){case "mmAPIReady":if(this.isAPIReady)return;this.isAPIReady=true;break;case "loadedmeta":this.loadedmeta=e;break;case "totalerror":this.totalerror=true;break;case "mediareset":this._bufferLoaded=this.totalerror=this.loadedmeta=false;break}if(!(!this.isAPIActive||this.totalerror&&!u[j])){!this.isAPIReady&&!u[j]&&this._trigger("mmAPIReady");if(c.type==="progresschange"){if(isFinite(this._bufferLoaded)&&isFinite(c.relLoaded)&&this._bufferLoaded>=c.relLoaded)return;
this._bufferLoaded=c.relLoaded;c.relStart=c.relStart||0;if(this._concerningBufferStart!==c.relStart){this._concerningBufferStart=c.relStart;this._trigger({type:"bufferrange",relStart:c.relStart,relLoaded:c.relLoaded})}}c.target=this.element;c=a.Event(c);c.preventDefault();e.mediaAPI=this.name;x[j]&&c.stopPropagation();a.event.trigger(c,e,this.element)}},supportsFullScreen:function(){return this._videoFullscreen||false},enterFullscreen:a.noop,exitFullscreen:a.noop,isAPIReady:false,isJMEReady:function(){return this.isAPIReady},
...
enterFullScreen:function(){if(!this._videoFullscreen)return false;try{this.element[k.enter]()}catch(c){}},exitFullScreen:function(){if(!this._videoFullscreen)return false;try{this.element[k.exit]()}catch(c){}}},d));t.add("nativ","audio",d);a.fn.getJMEAPI=function(c){if(this[0]){var e=a.data(this[0],"mediaElemSupport");return c||!e||!e.name||!e.apis?e:e.apis[e.name]}};a.fn.getMMAPI=a.fn.getJMEAPI;var h={onAPIReady:1,loadSrc:1};t.registerAPI=function(c){if(typeof c==="string")c=[c];a.each(c,function(e,
j){if((e=t.apis.video.nativ[j])&&!a.fn[j]&&a.isFunction(e)&&j.indexOf("_")!==0)a.fn[j]=function(){var q=arguments,m;this.each(function(){var l=a(this).getJMEAPI();if(l)if(h[j]||l.isJMEReady()&&!l.totalerror&&(l.name!=="nativ"||a.support.mediaElements))m=l[j].apply(l,q);else{l.unAPIReady(j+"queue");l.onAPIReady.call(l,function(){l[j].apply(l,q)},j+"queue")}});return m===undefined?this:m}})};var f=[];a.each(t.apis.video.nativ,function(c){f.push(c)});t.registerAPI(f);t.fn._extend=function(c){var e=[];
a.each(c,function(j,q){t.fn[j]=q;e.push(j)});t.registerAPI(e)}})(jQuery);
