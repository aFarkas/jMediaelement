...
"loop","controls","poster","preload"],function(q,t){n[t]=a.attr(c,t)});break;case "preload":n=c.preload||c.getAttribute("preload");if(n==="")n="auto";s[n]||(n="metadata");break}return n}else if(p[e]){g=!!g;c[e]=g;e==="autoplay"&&u.changeAutoplay(c,g);if(g){c[e]=g;c.setAttribute(e,e)}else{c[e]=g;c.removeAttribute(e)}}else if(l[e])c.setAttribute(e,g);else if(e==="srces"){a("source, a.source",c).remove();c.removeAttribute("src");g=a.isArray(g)?g:[g];u.addSrces(c,g,m)||a.each(g,function(q,t){n=f.createElement("source");
if(typeof t==="string")t={src:t};n.setAttribute("src",t.src);t.type&&n.setAttribute("type",t.type);t.media&&n.setAttribute("media",t.media);c.appendChild(n)})}else if(e==="getConfig")a.each(g,function(q,t){a.attr(c,q,t)});else if(e==="preload"){if(g==="")g="auto";else s[g]||(g="metadata");u.change(c,g,m);c.setAttribute(e,g)}};a.event.special.mediaerror={setup:function(){var c=a(this).bind("error",a.event.special.mediaerror.handler).bind("native_mediareset",x);setTimeout(function(){c.each(x)},0)},
...
c=c[c.length-1].src.split("?")[0];return c.slice(0,c.lastIndexOf("/")+1)}(),extendWithData:function(){var c={string:1,number:1,"boolean":1};return function(e,g,m){m||(m=g);a.each(m,a.isArray(m)?function(n,q){r.getData(e,q,g)}:function(n,q){c[typeof q]&&r.getData(e,n,g)});return g}}(),getData:function(){var c=function(e,g){e=e.getAttribute("data-"+g);if(!(!e&&e!==""))return e*1==e?parseFloat(e,10):e==="false"?false:e==="true"?true:e};return function(e,g,m){var n=g;m=m||{};if(typeof g==="string")g=
[n];a.each(g,a.isArray(g)?function(q,t){q=c(e,t);if(q!==undefined)m[t]=q}:function(q){var t=c(e,q);if(t!==undefined)m[q]=t});return m}}(),registerMimetype:function(c,e){arguments.length===1?a.each(z,function(g){r.registerMimetype(g,c)}):a.each(e,function(g,m){if(z[c][g])z[c][g]=[];z[c][g]=z[c][g].concat(m)})},_showMimeTypes:function(){window.console&&console.log(z)},add:function(c,e,g){if(!this.apis[e][c]){this.apis[e][c]=r.beget(this.fn);c!=="nativ"&&a.inArray(c,a.fn.jmeEmbed.defaults.apiOrder)===
-1&&a.fn.jmeEmbed.defaults.apiOrder.push(c)}a.extend(true,this.apis[e][c],g)},fn:{_init:a.noop,canPlayType:function(c){var e=this.apiElem,g;if(e&&e.canPlayType){g=e.canPlayType(c);return g==="no"?"":g}c=r.extractContainerCodecsFormType(c);var m=this;g="probably";if(!c[1])return this.canPlayContainer&&a.inArray(c[0],this.canPlayContainer)!==-1?"maybe":"";a.each(c[1],function(n,q){if(!m.canPlayCodecs||a.inArray(q,m.canPlayCodecs)===-1){g="";return false}});return g},canPlaySrc:function(c){var e=this;
if(typeof c!=="string"){if(c.type)return this.canPlayType(c.type);c=c.src}var g=A(c),m="";a.each(z[this.nodeName],function(n,q){if(a.inArray(g,q)!==-1){m=e.canPlayType(n);return false}});return m},canPlaySrces:function(c){c=c||a.attr(this.element,"srces");a.isArray(c)||(c=[c]);var e=this,g=false,m="";a.each(c,function(n,q){if(g=e.canPlaySrc(q)){m=q;return false}});return m},_setActive:a.noop,_setInactive:a.noop,_trigger:function(c){a(this.element).triggerHandler(c,c)}},apis:{audio:{},video:{}},extractContainerCodecsFormType:function(c){c=
...
width:0,visibility:"hidden"});g.apis[m]._setInactive(e);g.apis[m].isAPIActive=false;g.apis[e]._trigger({type:"apiDeActivated",api:m})}g.name=e;return t},getSuitedPlayers:function(c,e){var g=a.data(c,"mediaElemSupport");if(g&&g.apis){g=g.apis;var m=a.attr(c,"srces"),n=false,q=function(t,v){if(!(typeof v.isTechAvailable==="boolean"&&!v.isTechAvailable||a.isFunction(v.isTechAvailable)&&!v.isTechAvailable())){if(v=v.canPlaySrces(m))n={src:v.src||v,name:t};return n}};if(!m.length)return"noSource";e?a.each(e,
function(t,v){return!q(v,g[v])}):a.each(g,function(t,v){return!q(t,v)});return n}},_embedApi:function(c,e,g){var m=a.attr(c,"getConfig"),n=a(c),q=c.id,t=n.attr("aria-labelledby");if(!q){i++;q=g.nodeName+"-"+i;c.id=q}g.apis[e.name].visualElem=a('<div class="media-element-box mm-'+g.nodeName+'-box" style="position: relative;" />').insertBefore(c);t&&g.apis[e.name].visualElem.attr({role:"group","aria-labelledby":t});if(g.nodeName==="audio"&&!m.controls)g.apis[e.name].visualElem.css({height:0,width:0});
...
".").match(/(\d+)/g)||["0"])&&e[0]){c[0]=e[0];if(e[1])c[0]+="."+e[1];c[0]=parseFloat(c[0],10);if(e[2])c[1]=parseInt(e[2],10)}return c},embedObject:function(c,e,g,m,n){c.style.overflow="hidden";c=a("<div />").prependTo(c)[0];var q;if(navigator.plugins&&navigator.plugins.length){q=f.createElement("object");a.each(g,function(t,v){q.setAttribute(t,v)});a.each(m,function(t,v){var C=f.createElement("param");C.setAttribute("name",t);C.setAttribute("value",v);q.appendChild(C)});q.setAttribute("id",e);q.setAttribute("name",
e);if(m.wmode==="transparent"){q.style.minHeight="1px";q.style.minHeight="1px"}c.parentNode.replaceChild(q,c)}else if(window.ActiveXObject){q='<object style="width: 100%; height: 100%; width="100%" height="100%"';a.each(a.extend({},g,n),function(t,v){q+=" "+t+'="'+v+'"'});q+=' name="'+e+'"';q+=' id="'+e+'"';q+=">";a.each(m,function(t,v){q+=' <param name="'+t+'" value="'+v+'" />'});q+="</object>";c.outerHTML=q;q=f.getElementById(e)}if(q){q.setAttribute("width","100%");q.setAttribute("height","100%")}a(window).unload(function(){jQuery.cleanData([q]);
q=null});setTimeout(function(){if(q&&q.style){q.style.width="100%";q.style.height="100%"}},0);return q}});r.add("nativ","video",{});r.add("nativ","audio",{});var B=function(){var c=a(this).hide().children(":not(source, track)").clone().insertAfter(this);a(this).one("mediareset",function(){a(this).show();c.remove()})};a.fn.jmeEmbed=function(c){c=a.extend(true,{},a.fn.jmeEmbed.defaults,c);c.showFallback&&a.support.mediaElements&&this.bind("totalerror",B);return this.each(function(){var e=this.nodeName.toLowerCase(),
g=false,m=this;if(!(e!=="video"&&e!=="audio"||a.support.flash9&&a.nodeName(m.parentNode,"object"))){a("object, embed",this).each(function(){a("> *:not(param, embed, object)",this).appendTo(m)}).remove();a(this).trigger("jmeBeforeEmbed",{options:c,nodeName:e});c.removeControls&&a.attr(this,"controls",false);var n=r._create(e,"nativ",this,c);n.name="nativ";n.apis.nativ.apiElem=this;n.apis.nativ.visualElem=a(this);n.apis.nativ.isAPIActive=true;a.each(r.apis[e],function(q){q!=="nativ"&&r._create(e,q,
...
100||d.element.readyState===1)clearInterval(o)},o;a(d.element).bind("progress load",l);"buffered"in d.element&&a(d.element).bind("play waiting loadstart",function(){clearInterval(o);if(d.isAPIActive){o=setInterval(p,333);p()}})},b={isTechAvailable:a.support.mediaElements,_init:function(){var d=this,j=this.apiElem.muted,l=false,p=function(o){clearInterval(l);if(!d.element.error&&d.element.mozLoadFrom&&d.isAPIActive&&!d.element.readyState&&d.element.networkState===2&&a.support.flash9)if(o===true)d.element.load();
...
"mute-unmute":{stateMethod:"muted",actionMethod:"toggleMuted",evts:"mute loadedmeta",trueClass:"ui-icon-volume-off",falseClass:"ui-icon-volume-on"}},function(i,f){a.fn.jmeControl.addControl(i,function(b,h,k,d){function j(p){if(i=="play-pause"&&p&&p.type=="playing"?true:h[f.stateMethod]()){l.text.text(l.names[1]);l.title.attr("title",l.titleText[1]);l.icon.addClass(f.trueClass).removeClass(f.falseClass)}else{l.text.text(l.names[0]);l.title.attr("title",l.titleText[0]);l.icon.addClass(f.falseClass).removeClass(f.trueClass)}}
var l=a.fn.jmeControl.getBtn(b);d.addThemeRoller&&b.addClass("ui-state-default ui-corner-all");j();h.bind(f.evts,j).jmeReady(j);b.bind("ariaclick",function(p){h[f.actionMethod]();p.preventDefault()})})});a.each(["current-time","remaining-time"],function(i,f){a.fn.jmeControl.addControl(f,function(b,h,k,d){var j=f=="remaining-time"?function(p,o){b.html(k.apis[k.name]._format(l-o.time))}:function(p,o){b.html(k.apis[k.name]._format(o.time))},l=Number.MIN_VALUE;d.addThemeRoller&&b.addClass("ui-widget-content ui-corner-all");
b.html("00:00").attr("role","timer");f=="remaining-time"&&h.bind("loadedmeta",function(p,o){l=o.duration||Number.MIN_VALUE;j(false,{time:0})});h.bind("timechange",j).bind("mediareset",function(){b.html("00:00")})})});a.fn.jmeControl.addControls([{name:"duration",fn:function(i,f,b,h){h.addThemeRoller&&i.addClass("ui-widget-content ui-corner-all");i.html("00:00");f.bind("loadedmeta",function(k,d){i.html(b.apis[b.name]._format(d.duration))}).bind("mediareset",function(){i.html("00:00")})}},{name:"media-controls",
options:{dynamicTimeslider:false,timeSliderAdjust:0,excludeSel:false,fullWindowOverlay:false},fn:function(i,f,b,h){function k(){var l=i.width()+h.mediaControls.timeSliderAdjust,p=d.outerWidth(true)-d.innerWidth();a("> *",i).each(function(){if(d[0]!==this&&this.offsetWidth&&a.curCSS(this,"position")!=="absolute"&&(!h.excludeSel||!a(this).is(h.excludeSel)))p+=a(this).outerWidth(true)});d.css("width",l-p)}h.addThemeRoller&&i.addClass("ui-widget ui-widget-header ui-corner-all");i.attr("role","toolbar");
...
options:f,fn:function(b,h,k,d){var j=false,l=function(){if(k.apis[k.name].loadedmeta&&k.apis[k.name].loadedmeta.duration){b[i]("option","step",100/Math.max(100,b[0].offsetWidth));b[i]("option","disabled",false)}else b[i]("option","disabled",true)};b[i](d.timeSlider)[i]("option","disabled",true);a(window).bind("resize",l);a(document).bind("emchange",l);h.bind("loadedmeta resize",l).bind("timechange",function(p,o){parseInt(o.timeProgress,10);o.timeProgress!==undefined&&!j&&b[i]("value",o.timeProgress)}).bind("mediareset",
function(){b[i]("value",0);l()}).bind("ended",function(){b[i]("value",100)});b.bind("slidestart",function(p){if(p.originalEvent)j=true}).bind("slidestop",function(){j=false}).bind("slide",function(p,o){p.originalEvent&&k.apis[k.name].isAPIReady&&k.apis[k.name].relCurrentTime(o.value)})}},{name:"volume-slider",options:a.extend({mutestate:false},f),fn:function(b,h,k,d){function j(p,o){l||(p.type=="volumelevelchange"?b[i]("value",o.volumelevel):b[i]("value",h.muted()?0:h.volume()))}var l=false;b[i](d.volumeSlider)[i]("option",
"disabled",true);b.bind("slidestart",function(p){if(p.originalEvent)l=true}).bind("slidestop",function(){l=false}).bind("slide",function(p,o){if(p.originalEvent&&k.apis[k.name].isAPIReady){k.apis[k.name].volume(o.value);d.volumeSlider.mutestate&&k.apis[k.name].muted()&&k.apis[k.name].muted(false)}});h.bind("volumelevelchange loadedmeta",j).jmeReady(function(){b[i]("option","disabled",false);j({type:"ready"})});d.volumeSlider.mutestate&&h.bind("mute",j)}},{name:"progressbar",fn:function(b,h,k,d){function j(p,
...
h.replace(r.A,"%26").replace(r.a,"%26").replace(r.e,"%3D").replace(r.q,"%3F"):h};(function(){a.support.flash9=false;a.support.flashVersion=0;var h=y.getPluginVersion("Shockwave Flash"),k=function(j){a.support.flash9=false;try{if(j&&typeof j.GetVariable!=="undefined"){var l=j.GetVariable("$version");j=y.getPluginVersion("",{description:l});a.support.flashVersion=j[0];a.support.flash9=!!(j[0]>9||j[0]===9&&j[1]>=115)}}catch(p){}};if(h&&h[0])a.support.flashVersion=h[0];if(h[0]>9||h[0]===9&&h[1]>=115){a.support.flash9=
true;a(function(){h=a("<object />",w).appendTo("body");k(h[0]);h.remove()})}else if(window.ActiveXObject)try{h=new ActiveXObject("ShockwaveFlash.ShockwaveFlash");k(h);h=null}catch(d){}})();var b={isTechAvailable:function(){return a.support.flash9},_extendJWLoad:function(h,k,d){if(h){d=d||this.element;var j=this.embedOpts.jwPlayer.changeVars;y.extendWithData(d,k,["type","provider","stretching","bufferlength","streamer"]);k.file=(d.getAttribute("data-jwprefixsrc")||"")+k.file;var l=y.getExt(h),p=l==
"m4r"?"video":this.nodeName;if(l=="m4r"||!this.canPlaySrc(h)){if(!k.provider)k.provider=i[p];if(!k.type)k.type=i[p]}j&&j(h,k,d,this);return k}},_embed:function(h,k,d,j){var l=this.embedOpts.jwPlayer,p=a.extend({},l.vars,{file:h,id:k}),o=a.extend({name:k},l.attrs,w,!window.ActiveXObject?{data:l.path}:{}),s=a.extend({movie:l.path},l.params),u=[],A=this;this._extendJWLoad(h,p);if(d.poster)p.image=d.poster;p.autostart=""+d.autoplay;p.repeat=d.loop?"single":"false";p.controlbar=d.controls?"bottom":"none";
if(!d.controls&&this.nodeName!=="audio"&&s.wmode===undefined)s.wmode="transparent";if(!d.controls&&l.hideIcons&&s.wmode==="transparent"||l.hideIcons===true){p.icons="false";p.showicons="false"}if(s.wmode==="transparent"&&!p.screencolor&&!o.bgcolor){p.screencolor="ffffffff";o.bgcolor="#000000"}s.flashvars=[];a.each(p,function(z,B){s.flashvars.push(f(z)+"="+f(B))});a.each(l.plugins,function(z,B){u.push(B)});u.length&&s.flashvars.push("plugins="+u.join(","));s.flashvars=s.flashvars.join("&");j(y.embedObject(this.visualElem[0],
k,o,s,x,"Shockwave Flash"));setTimeout(function(){var z=a("object",A.visualElem)[0];if(!z||z.style.display==="none"&&a("> *",A.visualElem).length>1){a("div[bgactive]",A.visualElem).css({width:"100%",height:"100%"});A._trigger("flashblocker")}},9)},canPlaySrc:function(h){var k=y.fn.canPlaySrc.apply(this,arguments),d=-1;d=h.src||h;if(!k&&typeof d==="string"){d=d.indexOf("youtube.com/");if(d<15&&d>6)k="maybe"}return k},canPlayCodecs:["avc1.42E01E","mp4a.40.2","avc1.58A01E","avc1.4D401E","avc1.64001E",
"VP6","mp3","AAC"],canPlayContainer:["video/3gpp","video/x-msvideo","video/quicktime","video/x-m4v","video/mp4","video/m4p","video/x-flv","video/flv","audio/mpeg","audio/aac","audio/mp4","audio/x-m4a","audio/m4a","audio/mp3","audio/x-fla","audio/fla","youtube/flv","jwplayer/jwplayer"]};y.add("jwPlayer","video",b);y.add("jwPlayer","audio",b)})(jQuery);
