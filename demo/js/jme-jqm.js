		...
		$.jme.getButtonText = function(button, classes){
			...
			return function(state){
				...
				if(doTitle){
					...
					/* Added to update the label element's title. For instance when using jQuery Mobile's Checkbox and Flip Toggle Switch controls. -mderting */
					if (btnLabelElem[0]) {
	...
	$.jme.registerPlugin('poster-display', {
		...
		_create: function(control, media, base, options){
			...
			var showPoster = function(e){
				if(options.restoreOnEnd){
					control.fadeIn('slow')
				}
			};
			
			var hidePoster = function(e){
				if(options.restoreOnEnd){
					control.fadeOut('slow')
				}
			};
			
			media.bind('emptied ended', showPoster);
			media.bind('play', hidePoster);

			control
				.jmeProp('restoreOnEnd', options.restoreOnEnd)
			;
		}
		
	});

	$.jme.defineProp('restoreOnEnd', {
		set: function(elem, value){
			var data = $.jme.data(elem);
			data.restoreOnEnd = !!value;
			$(elem).triggerHandler('posterupdate');
			data.player.triggerHandler('posterupdate');
			return 'noDataSet';
		}
	});
