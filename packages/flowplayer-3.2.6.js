...
// {{{ public methods 
		...
		load: function(fn) { 
			if (!self.isLoaded() && self._fireEvent("onBeforeLoad") !== false) {
				var onPlayersUnloaded = function() {
					...
					//console.log('flowplayer load. wrapper is: ' + wrapper);
					//console.log('flowplayer load. wrapper.innerHTML is: ' + wrapper.innerHTML);
					//console.log('flowplayer load. params: ' + JSON.stringify(params));
					//console.log('flowplayer load. conf: ' + JSON.stringify(conf));
	...
	each(("Click*,Load*,Unload*,Keypress*,Volume*,Mute*,Unmute*,PlaylistReplace,ClipAdd,Fullscreen*,FullscreenExit,Error,MouseOver,MouseOut").split(","),
		function() {		 
			var name = "on" + this;
			//console.log('flowplayer js event handlers: name is: ' + name);
			// before event
			if (name.indexOf("*") != -1) {
				name = name.slice(0, name.length -1);
				//console.log('before event. flowplayer js event handlers: name is: ' + name); 
			...
			self[name] = function(fn) {
			  //console.log('normal event. flowplayer js event handlers: name is: ' + name);
...
(function() {
	...
	if (JQUERY) {
		...
		jQuery.fn.flashembed = function(opts, conf) {		
		  //console.log('jQuery.fn.flashembed');
		  //console.log('jQuery.fn.flashembed. this is: ' + this + ' opts is: ' + JSON.stringify(opts) + ' conf is: ' + JSON.stringify(conf));
			return this.each(function() { 
