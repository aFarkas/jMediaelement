...
getFormattedTime:function(){return this._format(this.currentTime())},loadSrc:function(d,j,m,q){if(d){a.attr(this.element,"srces",d);d=a.isArray(d)?d:[d]}else d=a.attr(this.element,"srces");if(j!==undefined)j?a.attr(this.element,"poster",j):a(this.element).removeAttr("poster");else j=a.attr(this.element,"poster");if(typeof m=="string"){var o=a.data(this.element,"mediaElemSupport");o.mediaName&&o.mediaName.text(m)}this._isResetting=true;d=this.canPlaySrces(d);this._trigger("mediareset");if(d){d=d.src||
d;this._mmload(d,j,q)}else{w._setAPIActive(this.element,"nativ");this._trigger("native_mediareset");a(this.element).data("mediaElemSupport").apis.nativ._mmload()}this._isResetting=false},isPlaying:function(){return this._isResetting?false:this._isPlaying()},_makenum:function(d){var j=false;if(d==d*1)j=parseFloat(d,10);return j}});var f=function(d){var j=function(){for(var r=d.element.currentTime||0,u=d.element.buffered,B=u.length,A={},z=0;z<B;z++){A.start=u.start(z);A.end=u.end(z);if(A.start<=r&&
A.end>=r)break}return A},m=function(r){var u={type:"progresschange"},B;if(this.buffered&&this.buffered.length){if(r=this.duration){B=j();u.relStart=B.start/r*100;u.relLoaded=B.end/r*100}d._trigger(u)}else if(r.originalEvent&&"lengthComputable"in r.originalEvent&&r.originalEvent.loaded){if(r.originalEvent.lengthComputable&&r.originalEvent.total){u.relStart=0;u.relLoaded=r.originalEvent.loaded/r.originalEvent.total*100}d._trigger(u)}if(!u.relLoaded&&this.readyState===4){u.relStart=0;u.relLoaded=100;
d._trigger(u)}return u.relLoaded},q=function(){if(m.call(d.element,{type:"ipadprogress"})>=100||d.element.readyState===1)clearInterval(o)},o;a(d.element).bind("progress load",m);"buffered"in d.element&&a(d.element).bind("play waiting loadstart",function(){clearInterval(o);if(d.isAPIActive){o=setInterval(q,333);q()}})},c={isTechAvailable:a.support.mediaElements,_init:function(){var d=this,j=this.apiElem.muted,m=false,q=function(o){clearInterval(m);if(!d.element.error&&d.element.mozLoadFrom&&d.isAPIActive&&
!d.element.readyState&&d.element.networkState===2&&a.support.flash9)if(o===true)d.element.load();else m=setTimeout(function(){q(true)},o==="initial"?2E4:9E3)};f(this);a(this.element).bind({volumechange:function(){if(j!==d.apiElem.muted){j=d.apiElem.muted;d._trigger.call(d,{type:"mute",isMuted:j})}else d._trigger.call(d,{type:"volumelevelchange",volumelevel:d.apiElem.volume*100})},ended:function(){if(d.isAPIActive&&this.ended&&!this.paused&&!a.attr(this,"loop")){d._stopEvent("pause",true);this.pause()}},
timeupdate:function(){var o={type:"timechange",time:this.currentTime};if(this.duration){o.duration=this.duration;o.timeProgress=o.time/o.duration*100}d._trigger(o)},progress:function(){!d.loadedmeta&&this.duration&&d._trigger({type:"loadedmeta",duration:this.duration})},loadedmetadata:function(){d._trigger({type:"loadedmeta",duration:this.duration})}}).bind("play pause playing waiting ended",function(o,r){if(!(!d.isAPIActive||d.totalerror||o.bubbles)){var u=this.parentNode||this.ownerDocument;if(!o.isPropagationStopped()&&
u){o.bubbles=true;r=a.makeArray(r);r.unshift(o);a.event.trigger(o,r,u,true)}}}).bind("play playing",function(o){if(!d.isAPIActive&&o.originalEvent&&!d.element.paused&&!d.element.ended)try{d.element.pause()}catch(r){}}).bind("mediareset",q).bind("ended play pause waiting playing",function(o){if(!d.isAPIActive&&o.originalEvent&&(o.originalEvent.mediaAPI==="nativ"||!o.originalEvent.mediaAPI)||d._stoppedEvents[o.type])o.stopImmediatePropagation()});q("initial");a.support.mediaLoop||a(this.element).bind("ended",
function(){if(d.isAPIActive&&a.attr(this,"loop")){var o=this;setTimeout(function(){a.attr(o,"loop")&&o.play()},0)}});if(!this.element.error)if(this.element.readyState>0)this._trigger({type:"loadedmeta",duration:this.element.duration});else if(this.element.networkState!==2||"webkitPreservesPitch"in this.element)this._trigger("mmAPIReady")},play:function(){this.element.play()},pause:function(){this._allowEvent("pause");this.element.pause()},muted:function(d){if(typeof d!=="boolean")return this.element.muted;
this.element.muted=d},volume:function(d){if(!isFinite(d))return this.element.volume*100;this.element.volume=d/100},currentTime:function(d){if(!isFinite(d))return this.element.currentTime;try{this.element.currentTime=d}catch(j){}},_mmload:function(){this.element.load?this.element.load():a(this.element).triggerHandler("error")},_isPlaying:function(){return!this.element.paused&&this.element.readyState>2&&!this.element.error&&!this.element.ended},getDuration:function(){return this.element.duration},getCurrentSrc:function(){return this.element.currentSrc}};
w.add("nativ","video",a.extend({_videoFullscreen:a.support.videoFullscreen,enterFullScreen:function(){if(!this._videoFullscreen)return false;try{this.element[h.enter]()}catch(d){return false}return true},exitFullScreen:function(){if(!this._videoFullscreen)return false;try{this.element[h.exit]()}catch(d){return false}return true}},c));w.add("nativ","audio",c);a.fn.getJMEAPI=function(d){if(this[0]){var j=a.data(this[0],"mediaElemSupport");return d||!j||!j.name||!j.apis?j:j.apis[j.name]}};var i={jmeReady:1,
getJMEVisual:1,jmeReady:1,isJMEReady:1,getMediaAPI:1,supportsFullScreen:1,loadSrc:1};w.registerAPI=function(d){if(typeof d==="string")d=[d];a.each(d,function(j,m){if((j=w.apis.video.nativ[m])&&!a.fn[m]&&a.isFunction(j)&&m.indexOf("_")!==0)a.fn[m]=function(){var q=arguments,o;this.each(function(){var r=a(this).getJMEAPI();if(r)if(i[m]||r.name=="nativ"&&m=="loadSrc"||r.isJMEReady()&&!r.totalerror&&(r.name!=="nativ"||a.support.mediaElements)){o=r[m].apply(r,q);return o===undefined}else{r.unAPIReady(m+
