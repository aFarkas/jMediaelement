...
"f4v"]}};a.extend(a.multimediaSupport,{registerMimetype:function(c,e){arguments.length===1?a.each(u,function(g){b.registerMimetype(g,c)}):a.each(e,function(g,i){if(u[c][g])u[c][g]=[];u[c][g]=u[c][g].concat(i)})},_showMimeTypes:function(){window.console&&console.log(u)},attrFns:{},add:function(c,e,g,i){if(!this.apis[e][c]||i){this.apis[e][c]=a.extend(this.apis[e][c]||{},this.apiProto,g);c!=="nativ"&&a.inArray(c,a.fn.mediaElementEmbed.defaults.apiOrder)===-1&&a.fn.mediaElementEmbed.defaults.apiOrder.push(c)}else a.extend(true,
this.apis[e][c],g)},apiProto:{_init:function(){},canPlayType:function(c){var e=this.apiElem;if(e&&e.canPlayType)return e.canPlayType(c);c=b.helper.extractContainerCodecsFormType(c);var g=this,i="probably";if(!c[1])return this.canPlayContainer&&a.inArray(c[0],this.canPlayContainer)!==-1?"maybe":"";a.each(c[1],function(o,k){if(!g.canPlayCodecs||a.inArray(k,g.canPlayCodecs)===-1){i="";return false}});return i},canPlaySrc:function(c){var e=this;if(typeof c!=="string"){if(c.type)return this.canPlayType(c.type);
c=c.src}var g=q(c),i="";a.each(u[this.nodeName],function(o,k){if(a.inArray(g,k)!==-1){i=e.canPlayType(o);return false}});return i},canPlaySrces:function(c){c=c||a.attr(this.html5elem,"srces");a.isArray(c)||(c=[c]);var e=this,g=false,i="";a.each(c,function(o,k){if(g=e.canPlaySrc(k)){i=k;return false}});return i},_setActive:function(){},_setInactive:function(){}},apis:{audio:{nativ:a.extend({},this.apiProto)},video:{nativ:a.extend({},this.apiProto)}},helper:{extractContainerCodecsFormType:function(c){c=
c.split(/\s*;\s*/g);if(c[1]&&c[1].indexOf("codecs")!==-1)c[1]=c[1].replace(/["|']$/,"").replace(/^\s*codecs=('|")/,"").split(/\s*,\s*/g);return c},makeAbsURI:function(){return function(c){if(c&&typeof c==="string")c=a('<a href="'+c+'"></a>')[0].href;return c}}(),beget:function(c){var e=function(){};e.prototype=c;return new e},_create:function(c,e,g,i){var o=a.data(g,"mediaElemSupport")||a.data(g,"mediaElemSupport",{apis:{},nodeName:c});if(!o.apis[e]){o.apis[e]=b.helper.beget(b.apis[c][e]);o.apis[e].html5elem=
g;o.apis[e].nodeName=c;o.apis[e].name=e;o.apis[e].data={};o.apis[e].embedOpts=i}return o},_setAPIActive:function(c,e){var g=a.data(c,"mediaElemSupport"),i=g.name;if(i===e)return true;var o=g.apis[i].apiElem,k=g.apis[e]&&g.apis[e].apiElem,n=false;if(k&&k.nodeName){if(g.nodeName!=="audio"||a.attr(c,"controls")){k.style.display="";e!=="nativ"&&g.apis[e].visualElem.css({width:a(g.apis[e].html5elem).width(),height:a(g.apis[e].html5elem).height(),visibility:""})}g.apis[e]._setActive(i);n=true}if(o&&o.nodeName){if(i===
"nativ")o.style.display="none";else g.apis[i].visualElem.css({height:0,width:0,overflow:"hidden",visibility:"hidden"});g.apis[i]._setInactive(e)}g.name=e;return n}},getSuitedPlayers:function(c,e){var g=a.data(c,"mediaElemSupport");if(g&&g.apis){g=g.apis;var i=a.attr(c,"srces"),o=false,k=function(n,r){if(!(!r.isTechAvailable||a.isFunction(r.isTechAvailable)&&!r.isTechAvailable())){if(r=r.canPlaySrces(i))o={src:r.src||r,name:n};return o}};if(!i.length)return"noSource";e?a.each(e,function(n,r){return!k(r,
g[r])}):a.each(g,function(n,r){return!k(n,r)});return o}},_embedApi:function(c,e,g){var i=a.attr(c,"getConfig"),o=a(c),k=c.id;if(!k){d++;k=g.nodeName+"-"+d;c.id=k}g.apis[e.name].visualElem=a('<div class="media-element-box mm-'+g.nodeName+'-box" />').insertAfter(c);g.nodeName==="audio"&&!i.controls?g.apis[e.name].visualElem.css({height:0,width:0,overflow:"hidden"}):g.apis[e.name].visualElem.css({width:o.width(),height:o.height()});g.apis[e.name]._embed(e.src,g.name+"-"+k,i,function(n){g.apis[e.name].apiElem=
n;a(n).addClass(g.nodeName);g.apis[e.name]._init()})},getPluginVersion:function(c){var e=navigator.plugins&&navigator.plugins[c];c=-1;if(e)if((e=(e.description||"").match(/(\d+\.\d+)/)||["0"])&&e[0])c=parseFloat(e[0],10);return c},embedObject:function(c,e,g,i,o){c=a("<div />").appendTo(c)[0];var k;if(!window.ActiveXObject||!c.outerHTML){k=f.createElement("object");a.each(g,function(n,r){k.setAttribute(n,r)});a.each(i,function(n,r){var x=f.createElement("param");x.setAttribute("name",n);x.setAttribute("value",
r);k.appendChild(x)});k.setAttribute("id",e);k.setAttribute("name",e);c.parentNode.replaceChild(k,c)}else{k="<object";a.each(a.extend({},g,o),function(n,r){k+=" "+n+'="'+r+'"'});k+=' name="'+e+'"';k+=' id="'+e+'"';k+=">";a.each(i,function(n,r){k+=' <param name="'+n+'" value="'+r+'" />'});k+="</object>";c.outerHTML=k;k=f.getElementById(e)}if(k){k.setAttribute("width","100%");k.setAttribute("height","100%");k.style.display.width="100%";k.style.display.height="100%"}a(window).unload(function(){jQuery.cleanData([k]);
k=null});return k}});a.fn.mediaElementEmbed=function(c){c=a.extend(true,{},a.fn.mediaElementEmbed.defaults,c);return this.each(function(){var e=this.nodeName.toLowerCase();if(!(e!=="video"&&e!=="audio")){var g=this;if(c.removeControls){this.controls=false;a(this).removeAttr("controls")}var i=b.helper._create(e,"nativ",this,c);i.name="nativ";i.apis.nativ.apiElem=this;a.each(b.apis[e],function(o){o!=="nativ"&&b.helper._create(e,o,g,c)});c.debug||!a.support.mediaElements||this.error?s(this,c):i.apis[i.name]._init();
a(this).bind("mediaerror",function(){i.name==="nativ"&&s(this,c)}).bind("loadedmetadata",m)}})};a.fn.mediaElementEmbed.defaults={debug:false,removeControls:false,apiOrder:[]};if(a.cleanData){var y=a.cleanData;a.cleanData=function(c){y(c);for(var e=0,g=c.length;e<g;e++)if(c[e].nodeName==="OBJECT")try{for(var i in c[e])if(typeof c[e][i]==="function")c[e][i]=null}catch(o){}}}})(jQuery);
...
"a11ySlider":"slider",s=/\s*\/\s*|\s*\|\s*/,m={"timeline-slider":function(b,d,f,h){function l(t,u){u.timeProgress!==undefined&&!v&&b[q]("value",u.timeProgress)}function j(){f.apis[f.name].loadedmeta&&f.apis[f.name].loadedmeta.duration?b[q]("option","disabled",false):b[q]("option","disabled",true)}var v=false;b[q](h.timeSlider)[q]("option","disabled",true);p(d,f.apis[f.name].isAPIReady,"mmAPIReady",function(){d.bind("emptied loadedmeta",j).bind("timechange",l);b.bind("slidestart",function(t){if(t.originalEvent)v=
true}).bind("slidestop",function(){v=false}).bind("slide",function(t,u){t.originalEvent&&f.apis[f.name].relCurrentTime(u.value)});j()},"one")},"volume-slider":function(b,d,f,h){function l(v,t){j||b[q]("value",t.volumelevel)}var j=false;b[q](h.volumeSlider)[q]("option","disabled",true);p(d,f.apis[f.name].isAPIReady,"mmAPIReady",function(){d.bind("volumelevelchange",l);b.bind("slidestart",function(v){if(v.originalEvent)j=true}).bind("slidestop",function(){j=false}).bind("slide",function(v,t){v.originalEvent&&
f.apis[f.name].volume(t.value)});b[q]("option","disabled",false);b[q]("value",f.apis[f.name].volume())},"one")},progressbar:function(b,d,f,h){function l(v,t){t.lengthComputable?b.progressbar("option","disabled",false).progressbar("value",t.relLoaded):b.progressbar("option","disabled",true)}function j(){b.progressbar("option","disabled",true).progressbar("value",0)}b.progressbar(h.progressbar).progressbar("option","disabled",true);p(d,f.apis[f.name].isAPIReady,"mmAPIReady",function(){d.bind("progresschange",
l).bind("emptied",j)},"one")},duration:function(b,d,f,h){h.addThemeRoller&&b.addClass("ui-widget-content ui-corner-all");b.html("--:--");d.bind("loadedmeta emptied",function(l,j){b.html(f.apis[f.name]._format(j.duration))});p(d,f.apis[f.name].isAPIReady,"mmAPIReady",function(){b.html(f.apis[f.name].getFormattedDuration())},"one")},"current-time":function(b,d,f,h){h.addThemeRoller&&b.addClass("ui-widget-content ui-corner-all");b.html("--:--");d.bind("timechange",function(l,j){b.html(f.apis[f.name]._format(j.time))});
p(d,f.apis[f.name].isAPIReady,"mmAPIReady",function(){b.html(f.apis[f.name].getFormattedTime())},"one")},"media-controls":function(b,d,f,h){function l(){var t=b.innerWidth()+h.mediaControls.timeSliderAdjust,u=j.outerWidth(true)-j.innerWidth();a("> *",b).each(function(){if(j[0]!==this&&this.offsetWidth)u+=a(this).outerWidth(true)});j.css("width",t-u)}h.addThemeRoller&&b.addClass("ui-widget ui-widget-header ui-corner-all");if(h.mediaControls.dynamicTimeslider){var j=a("."+h.classPrefix+"timeline-slider",
...
l,j){function v(){if(l.apis[l.name][d.stateMethod]()){u.text(y[1]);t.addClass(d.trueClass).removeClass(d.falseClass)}else{u.text(y[0]);t.addClass(d.falseClass).removeClass(d.trueClass)}}var t=a(".ui-icon",f),u=a(".button-text",f),y=u.text().split(s);j.addThemeRoller&&f.addClass("ui-state-default ui-corner-all");t[0]||(t=f);u[0]||(u=f);if(y.length<2)y=[y[0],y[1]];p(h,l.apis[l.name].isAPIReady,"mmAPIReady",function(){h.bind(d.evts,v);v()},"one");f.bind("click",function(c){l.apis[l.name][d.actionMethod]();
c.preventDefault()})}});a.fn.registerMMControl=function(b){function d(){var f=w(this,b);f.api&&f.controls.each(function(){var h=a(this);a.each(m,function(l,j){if(h.hasClass(b.classPrefix+l)){j(h,f.mm,f.api,b);return false}})})}b=a.extend(true,{},a.fn.registerMMControl.defaults,b);b.controlSel=[];a.each(m,function(f){f!=="media-controls"&&b.controlSel.push("."+b.classPrefix+f)});b.controlSel.push("."+b.classPrefix+"media-controls");b.controlSel=b.controlSel.join(", ");return this.each(d)};a.fn.registerMMControl.defaults=
...
new ActiveXObject("ShockwaveFlash.ShockwaveFlash");if(!m)return;if((m=m.GetVariable("$version").match(/(\d+\,\d+)/))&&m[0])if(parseFloat(m[0].replace(",","."),10)>=9)a.support.flash9=true}catch(b){}return a.support.flash9},_embed:function(m,b,d,f){var h=this.embedOpts.jwPlayer;m=a.extend({},h.vars,{file:m,id:b});var l=a.extend({name:b,data:h.path},h.attrs,p),j=a.extend({movie:h.path},h.params);if(d.poster)m.image=d.poster;m.autostart=""+d.autoplay;m.repeat=d.loop?"single":"false";m.controlbar=d.controls?
"bottom":"none";if((h.playFirstFrame||d.autobuffer)&&!d.poster&&!d.autoplay){this.data.playFirstFrame=true;m.autostart="true"}j.flashvars=[];a.each(m,function(v,t){j.flashvars.push(v+"="+t)});j.flashvars=j.flashvars.join("&");f(q.embedObject(this.visualElem[0],b,l,j,w))},canPlayCodecs:["avc1.42E01E","mp4a.40.2","avc1.58A01E","avc1.4D401E","avc1.64001E"],canPlayContainer:["video/x-msvideo","video/quicktime","video/x-m4v","video/mp4","video/m4p","video/x-flv","video/flv","audio/mpeg","audio/mp3","audio/x-fla",
...
1E3)},end:function(d){d._intervalCheckTimer&&clearInterval(d._intervalCheckTimer)}},m={2:"waiting",3:"playing",4:"pause",6:"ended"},b={_init:function(){p(this)},_intervalCheck:function(){var d=this.apiElem,f=d.input.state,h=this,l={type:"loadedmeta",duration:d.input.length/1E3},j;if(f&&f>1&&!this.data.metaLoaded||this.data.metaLoaded&&this.data.metaLoaded.duration!==l.duration){w(l,h);this.data.metaLoaded=l}j={type:"timechange",time:d.input.time/1E3};if(f&&f>1&&this.data.time!==j.time){if(l.duration){j.duration=
l.duration;j.timeProgress=d.input.position*100}this.data.time=j.time;w(j,h)}j={type:"mute",isMuted:d.audio.mute};if(j.isMuted!==this.data.isMuted){this.data.isMuted=j.isMuted;w(j,h)}j={type:"volumelevelchange",volumelevel:d.audio.volume/2};if(j.volumelevel!==this.data.volumelevel){this.data.volumelevel=j.volumelevel;w(j,h)}if(f!==this.data.state){this.data.state=f;m[f]&&w(m[f],h);m[f]==="ended"&&d.playlist.stop();if(f===3)s.start(h);else if(f===4||f===5||f===6||f===7)s.end(h)}},play:function(){this.apiElem.playlist.play();
