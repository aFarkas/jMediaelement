...
z)}}}function s(a){var b=a.indexOf("?"),d="";a=b>0?a.substring(0,b):a;b=a.lastIndexOf(".")+1;return d=a.substr(b)}function v(a,b){var d=a.nodeName.toLowerCase();b=j.getSuitedPlayers(a,b.apiOrder);var e=c.data(a,"mediaElemSupport");if(b){if(b!=="noSource")if(j._setAPIActive(a,b.name))e.apis[b.name]._mmload&&e.apis[b.name]._mmload(b.src,c.attr(a,"poster"));else j._embedApi(a,b,e,d)}else{e.apis.nativ._trigger({type:"totalerror"});try{a.pause()}catch(f){}}}c.multimediaSupport={};var j=c.multimediaSupport,
...
d,e){if(!(a.nodeName&&C.test(a.nodeName)&&(A[b]||x[b]||o[b])))return p(a,b,d,e);var f;if(d===undefined){if(x[b])return typeof a[b]==="boolean"?a[b]:!!(a.attributes[b]||{}).specified;if(o[b])return c.support.video&&a[b]||j.makeAbsURI(a.getAttribute(b));switch(b){case "srces":if(f=c.data(a,"jme-srces"))break;if(f=c.attr(a,"src"))f=[{src:f,type:a.getAttribute("type"),media:a.getAttribute("media")}];else{f=[];c("source",a).each(function(){f.push({src:c.attr(this,"src"),type:this.getAttribute("type"),
...
c.extend(j,{jsPath:function(){var a=c("script");a=a[a.length-1].src.split("?")[0];return a.slice(0,a.lastIndexOf("/")+1)}(),extendWithData:function(){var a={string:1,number:1,"boolean":1};return function(b,d,e){e||(e=d);c.each(e,c.isArray(e)?function(f,g){j.getData(b,g,d)}:function(f,g){a[typeof g]&&j.getData(b,f,d)});return d}}(),getData:function(){var a=function(b,d){b=b.getAttribute("data-"+d);if(!(!b&&b!==""))return b*1==b?parseFloat(b,10):b==="false"?false:b==="true"?true:b};return function(b,
d,e){var f=d;e=e||{};if(typeof d==="string")d=[f];c.each(d,c.isArray(d)?function(g,h){g=a(b,h);if(g!==undefined)e[h]=g}:function(g){var h=a(b,g);if(h!==undefined)e[g]=h});return e}}(),registerMimetype:function(a,b){arguments.length===1?c.each(y,function(d){j.registerMimetype(d,a)}):c.each(b,function(d,e){if(y[a][d])y[a][d]=[];y[a][d]=y[a][d].concat(e)})},_showMimeTypes:function(){window.console&&console.log(y)},add:function(a,b,d){if(!this.apis[b][a]){this.apis[b][a]=j.beget(this.fn);a!=="nativ"&&
